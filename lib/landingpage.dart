...
class _SelectaPageState extends State<SelectaPage> {
  ...
  final String _selectedFilter = 'All'; // Variable to store selected filter, set initial value to 'All'
  ...
  Future<int> getRandomQuizInformation(BuildContext context) async {
    // Calculate the random quiz index based on the available quizzes
    int randomQuizIndex =
        _Quiz_ID.isNotEmpty ? Random().nextInt(_Quiz_ID.length) : -1;

    if (randomQuizIndex != -1) {
      // Retrieve the quiz information based on the randomQuizIndex
      String quizID = _Quiz_ID[randomQuizIndex];
      bool quizTimed = _QuizTimed[randomQuizIndex];
      int timerTime = _TimerTime[randomQuizIndex];
      String quizName = _QuizName[randomQuizIndex];
      String quizDesc = _QuizDesc[randomQuizIndex];
      String quizCategory = _QuizCategory[randomQuizIndex];
      String quizType = _QuizType[randomQuizIndex];
      String numberOfQuestions = _NumberofQuestions[randomQuizIndex];
      String quizImage = _Quiz_Images[randomQuizIndex];
      String preReq = _QuizPrereq[randomQuizIndex];
    }
    return randomQuizIndex;
  }

  // Function to handle the random quiz button press
  Future<void> handleRandomQuizButtonPress(BuildContext context) async {
    //int randomQuizIndex = await getRandomQuizInformation(context);
    //int randomQuizIndex =_Quiz_ID.isNotEmpty ? Random().nextInt(_Quiz_ID.length) : -1;
  ...
  while (randomQuizIndex == -1 || _QuizPrereq[randomQuizIndex] != "none") {
    randomQuizIndex = _Quiz_ID.isNotEmpty ? Random().nextInt(_Quiz_ID.length) : -1;
  }    
          ...
          return AlertDialog(
            ...
            content: Column(
              children: [
                ...
                Text('Number of Questions: ${_NumberofQuestions[randomQuizIndex]}'),
              ],
            ),
            actions: [
              TextButton(
                      ...
                      child: Center(
                        child: Container(
                         // height: MediaQuery.of(context).size.height *
                          //   0.53, // Adjust the value as needed
